...
namespace aiprovider_openwebui\form;

use aiprovider_openwebui\helper;
use core_ai\form\action_settings_form;

/**
 * Base action settings form for OpenWebUI provider.
 *
...
class action_form extends action_settings_form {
    ...
    protected function definition(): void {
        ...
        $this->providername = $this->_customdata['providername'] ?? 'aiprovider_openwebui';
    ...
    public function validation($data, $files): array {
        ...
        if (!empty($data['modelextraparams'])) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                $errors['modelextraparams'] = get_string('invalidjson', 'aiprovider_openwebui');
            }
        }
    ...
    protected function add_model_fields(int $modeltype): void {
        global $PAGE;
        $PAGE->requires->js_call_amd('aiprovider_openwebui/modelchooser', 'init');
        $mform = $this->_form;
        ...
        $mform->addElement(
            'select',
            'modeltemplate',
            get_string("action:{$this->actionname}:model", 'aiprovider_openwebui'),
        ...
        $mform->addHelpButton('modeltemplate', "action:{$this->actionname}:model", 'aiprovider_openwebui');

        $mform->addElement('hidden', 'model', $defaultmodel);
        $mform->setType('model', PARAM_TEXT);

        $mform->addElement('text', 'custommodel', get_string('custom_model_name', 'aiprovider_openwebui'), ['size' => 55]);
        ...
        $mform->addRule('custommodel', get_string('required'), 'required', null, 'client');
