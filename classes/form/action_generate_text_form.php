...
use aiprovider_openwebui\aimodel\openwebui_base;
...
class action_generate_text_form extends action_form {
    ...
    protected function definition(): void {
        ...
        $this->add_model_fields(openwebui_base::MODEL_TYPE_TEXT);

        // API endpoint.
        $mform->addElement(
            'text',
            'endpoint',
            get_string("action:{$this->actionname}:endpoint", 'aiprovider_openwebui'),
            'maxlength="255" size="30"',
        );
        $mform->setType('endpoint', PARAM_URL);
        $mform->addRule('endpoint', null, 'required', null, 'client');
        $mform->setDefault('endpoint', $actionconfig['endpoint'] ?? '/api/v1/chat/completions');

        // System Instructions.
        $mform->addElement(
            'textarea',
            'systeminstruction',
            get_string("action:{$this->actionname}:systeminstruction", 'aiprovider_openwebui'),
        ...
        $mform->addHelpButton('systeminstruction', "action:{$this->actionname}:systeminstruction", 'aiprovider_openwebui');
